...
/**
 * @brief Test message output queue
 */
TEST_F(AlgorithmContextTest, ManagesOutputMessages) {
    // Initially empty
    EXPECT_TRUE(context->pending_outputs.empty());
    
    // Add messages
    messages::L2ToL1Message msg1, msg2;
    msg1.set_message_id("msg_001");
    msg2.set_message_id("msg_002");
    
    context->add_output_message(msg1);
    context->add_output_message(msg2);
    
    EXPECT_EQ(context->pending_outputs.size(), 2);
    EXPECT_EQ(context->pending_outputs[0].message_id(), "msg_001");
    EXPECT_EQ(context->pending_outputs[1].message_id(), "msg_002");
    
    // Clear messages
    context->pending_outputs.clear();
    EXPECT_TRUE(context->pending_outputs.empty());
}

/**
 * @brief Test message history management
 */
TEST_F(AlgorithmContextTest, ManagesMessageHistory) {
    // Initially empty
    auto messages = context->get_messages_from_node("test_node");
    EXPECT_TRUE(messages.empty());
    
    // Add message to history
    messages::L1ToL2Message l1_msg;
    l1_msg.set_message_id("l1_msg_001");
    l1_msg.set_source_node_id("test_node");
    
    context->add_message_to_history("test_node", l1_msg);
    
    // Retrieve messages
    auto retrieved = context->get_messages_from_node("test_node");
    EXPECT_EQ(retrieved.size(), 1);
    EXPECT_EQ(retrieved[0].message_id(), "l1_msg_001");
    
    // Test different node
    auto other_node_messages = context->get_messages_from_node("other_node");
    EXPECT_TRUE(other_node_messages.empty());
}
...
TEST_F(AlgorithmContextTest, MaintainsDataPersistence) {
    ...
    output.set_message_id("test_output");
